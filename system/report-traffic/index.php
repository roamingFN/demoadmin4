...
<body style="padding:10px;">
	<h1><a href="index.php">รานงานสถิติ</a></h1>
	...
	<table class="detail">
		<tr>
				<th>วันที่</th>
				<th>ยอดเติมเงิน(บาท)</th>
		</tr>
<?php

	$select_str = "select sum(topup_amount) as sum_topup_amount, DATE(topup_date) as date_topup_date
		from customer_request_topup where 1 ";
	...
	$select_str .= "group by DATE_FORMAT(topup_date, '%d-%m-%y') ";
	...
	if (mysql_num_rows($select_rate)>0) {
		...
		?>
				<tr>
					<td><?php echo $row['date_topup_date']; ?></td>
					<td><?php echo number_format($row['sum_topup_amount'],2); ?></td>
				</tr>
		<?php
		$current_row++;
		}

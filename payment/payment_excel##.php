<?php
	...
	if($stmt = $con->prepare('SELECT des FROM co_bank')){
		...
		if($stmt = $con->prepare($sql)){
			...
			header("Content-Disposition: attachment;filename=payment.xls "); // แล้วนี่ก็ชื่อไฟล์
			header("Content-Transfer-Encoding: binary ");

			xlsBOF();
			xlsCodepage();
			xlsWriteLabel(1,0,"Exported Payment Data");
			...
			while($stmt->fetch()){
				...
				xlsWriteLabel($xlsRow,12,iconv("utf-8","tis-620",$remarkc));
                                xlsWriteLabel($xlsRow,13,$topups[$topup_id]);
				$xlsRow++;
			}
			xlsEOF();
			exit();

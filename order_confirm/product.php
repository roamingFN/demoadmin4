...
<html>
	...
	<script>
		...
		function save(){
			...
			for(var i=0;i<save.length;i++) {
				...
				var quan = parseInt(document.getElementById('quan-'+id).value.replace(/,/g, ''));
				var cpp = document.getElementById('cpp-'+id).value.replace(/,/g, '');
				var tran = document.getElementById('tran-'+id).value.replace(/,/g, '');
				var stt1 = document.getElementById('stt1-'+id).checked;
				var stt2 = document.getElementById('stt2-'+id).checked;
				var totalp = numberify(document.getElementById('totalTh-'+id).textContent);
				var totalpCn = numberify(document.getElementById('totalCn-'+id).textContent);
				var comment = document.getElementById('rem-'+id).options[document.getElementById('rem-'+id).selectedIndex].value;
				var stt;
				var type = document.getElementById('type-'+id).options[document.getElementById('type-'+id).selectedIndex].value;
				var unconfirmed = quan;
				if (unconfirmed==0) {
					unconfirmed = document.getElementById('keepQuan-'+id).value;
				}
				var tTran = numberify(document.getElementById('tTran').textContent);
				var fquan = parseInt(document.getElementById('quan1-'+id).value);
				var tquan = numberify(document.getElementById('tquan').textContent);

				//validate quan and price
				if ((quan==='')||(cpp==='')) {
					alert("กรุณากรอกข้อมูลให้ครบทุกช่อง");
					return 0;		//exit
				}

				//validte quan with fquan
				if (quan>fquan) {
				...
				//order fail
					...
					stt = 2;
						if (comment==0) {
							alert("กรุณาเลือกหมายเหตุสำหรับสินค้าที่สั่งไม่ได้");
							return 0;
						}
						if (quan>0) {
							alert("จำนวนชิ้นสินค้าต้องเท่ากับ 0");
							return 0;
						}
				}
				else {
				...
				grandTotalTh = Number(grandTotalTh) + Number(totalp);
				grandTotalCn = Number(grandTotalCn) + Number(totalpCn);
			}
			data['oid'] = orderId;
		...
		function confirm(){
			...
			for(var i=0;i<save.length;i++) {
				...
				if ((quan==='')||(cpp==='')||((stt1==0)&&(stt2==0))||(type==0)) {
					alert("กรุณากรอกข้อมูลให้ครบทุกช่อง");
					return 0;		//exit
				}
			...
			data['oid'] = orderId;
			data['ono'] = orderNum;
			data['total'] = document.getElementById('tamountTh').textContent;
			data['grandTotalTh'] = grandTotalTh;
			data['grandTotalCn'] = grandTotalCn;
			data['tTran'] = tTran;
			data['tQuan'] = tquan;
			...
			xhr.onreadystatechange = function(){
				if(xhr.readyState==4 && xhr.status==200){
					if(xhr.responseText=='success'){
                        alert("update สถานะเรียบร้อยแล้ว");
						window.location = 'index.php';
					}else{
						//alert('กรุณาใส่ข้อมูลให้ถูกต้องค่ะ!');
						alert(xhr.responseText);
	...
	<body>
        ...
        <table class="order-results" style="width:800px;">
			...
			<tr class="punc">
				<td>สถานะรายการ :</td><td><?php echo $codes[$code1]; ?></td>
		...
		if ($code1==0) {
			foreach($shops as $key=>$item) {
				...
				echo '<thead>';
				if ($firstShop) {
					echo '<th width="5%"><button class="button-select" type="button" onclick="selectAll()">เลือกทั้งหมด</th>';
					$firstShop = 0;
				}
				...
				echo '<tbody class="shopname undivide">'.
					...
					'<th colspan="18">ร้าน '.base64_decode($key).'</th></tbody>';
				$shopp = $key;
				...
				for($i=0;$i<sizeof($item);$i++){
					...
					if ($o_status==0) {
						$option = '<div style="display:inline">'.
									'<input style="width:auto" type="checkbox" value="1" id="stt1-'.$opid.'" onclick="checkStat1('.$opid.')"><label> ได้</label>'.
								'</div>'.
								'&nbsp;&nbsp;&nbsp;<div style="display:inline">'.
									'<input style="width:auto" type="checkbox" value="2" id="stt2-'.$opid.'" onclick="checkStat2('.$opid.')"><label> ไม่ได้</label>'.
								'</div>';
						$optionRmk = '<select class="search-select" disabled id="rem-'.$opid.'">';
						$ecQuan1 = '<input disabled style="text-align:right;" class="tquan" id="quan1-'.$opid.'" value="'.$item[$i][27].'" onkeyup="this.value=numberifyWithOutDot(this.value);" onclick="checkZero(this.value,\'quan1-'.$opid.'\')" />';
						if ($_adminFlg==1) {
								$ecQuan1 = '<input style="text-align:right;" class="tquan" id="quan1-'.$opid.'" value="'.$item[$i][27].'" onkeyup="this.value=numberifyWithOutDot(this.value);" onclick="checkZero(this.value,\'quan1-'.$opid.'\')" />';
						}
						$ecQuan = '<input style="text-align:right;" class="tquan" id="quan-'.$opid.'" value="'.$item[$i][2].'" onkeyup="this.value=numberifyWithOutDot(this.value);calc1(\''.$item[$i][0].'\');" onclick="checkZero(this.value,\'quan-'.$opid.'\')" />';
						$ecCpp = '<input style="text-align:right;" id="cpp-'.$opid.'" value="'.number_format($item[$i][23],2).'" onkeyup="this.value=numberify(this.value);calc1(\''.$item[$i][0].'\');" onclick="checkZero(this.value,\'cpp-'.$opid.'\')"/>';
						$ecTran = '<input style="text-align:right;" class="tTran" id="tran-'.$opid.'" value="'.number_format($cncost,2).'" onkeyup="this.value=numberify(this.value);calc1(\''.$item[$i][0].'\');" onclick="checkZero(this.value,\'tran-'.$opid.'\')" />';
					}
					else if ($o_status==1) {
						$option = '<div style="display:inline">'.
									'<input style="width:auto" type="checkbox" value="1" id="stt1-'.$opid.'" onclick="checkStat1('.$opid.')" checked><label> ได้</label>'.
								'</div>'.
								'&nbsp;&nbsp;&nbsp;<div style="display:inline">'.
									'<input style="width:auto" type="checkbox" value="2" id="stt2-'.$opid.'" onclick="checkStat2('.$opid.')"><label> ไม่ได้</label>'.
								'</div>';
						...
						$option = '<div style="display:inline">'.
								...
								'</div>'.
								'&nbsp;&nbsp;&nbsp;<div style="display:inline">'.
									'<input style="width:auto" type="checkbox" value="2" id="stt2-'.$opid.'" onclick="checkStat2('.$opid.')" checked><label> ไม่ได้</label>'.
								'</div>';
						$optionRmk = '<select class="search-select" id="rem-'.$opid.'">';
						$ecQuan1 = '<input style="text-align:right;" class="tquan" id="quan1-'.$opid.'" value="'.$item[$i][27].'" onkeyup="this.value=numberifyWithOutDot(this.value);" onclick="checkZero(this.value,\'quan1-'.$opid.'\')" disabled />';
						$ecQuan = '<input style="text-align:right;" class="tquan" id="quan-'.$opid.'" value="'.$item[$i][2].'" onkeyup="this.value=numberifyWithOutDot(this.value);calc1(\''.$item[$i][0].'\');" onclick="checkZero(this.value,\'quan-'.$opid.'\')" disabled />';
						$ecCpp = '<input style="text-align:right;" id="cpp-'.$opid.'" value="'.number_format($item[$i][23],2).'" onkeyup="this.value=numberify(this.value);calc1(\''.$item[$i][0].'\');" onclick="checkZero(this.value,\'cpp-'.$opid.'\')" disabled />';
						$ecTran = '<input style="text-align:right;" class="tTran" id="tran-'.$opid.'" value="'.number_format($cncost,2).'" onkeyup="this.value=numberify(this.value);calc1(\''.$item[$i][0].'\');" onclick="checkZero(this.value,\'tran-'.$opid.'\')" disabled />';
					}
					...
					echo '<tr class="'.($puncCount%2==0? 'punc ':'').($o_status==0? 'cancel ':'').'">'.
						...
						'<td>'.$item[$i][18].'</td>'.
						'<td>'.$item[$i][19].'</td>'.
						'<td><select class="search-select" id="type-'.$opid.'">';
						foreach ($_ptype as $key => $value) {
								if($key==$item[$i][25]) echo '<option value="'.$key.'" selected>'.$value.'</option>';
								else echo '<option value="'.$key.'">'.$value.'</option>';
						}
						echo '</select></td>'.
						//'<td align="right" id="fquan-'.$opid.'">'.$item[$i][27].'</td>'.	//customer quan
						'<td>'.$ecQuan1.'</td>'.		//customer quan
						'<td>'.$ecQuan.'</td>'.			//quanltity
						'<td id="cpp1-'.$opid.'" align="right">'.number_format($item[$i][26],2).'</td>'.
						'<td>'.$ecCpp.'</td>'.	//price
						'<td>'.$ecTran.'</td>'.											//transport china cost
						...
						echo '</select></td>'.
						'<input type="hidden" id="keepQuan-'.$opid.'" value='.$item[$i][8].'>'.
						'<input type="hidden" id="keepTran-'.$opid.'" value='.$cncost.'>'.
						'<input type="hidden" id="shopname-'.$opid.'" value="'.$shopp.'" />'.
						'</tr>';
						$puncCount++;
						$no++;
				}
		...
		echo '<div style="text-align:center;overflow:hidden;position:relative;display:table;width:100%;">';
				...
				echo '<div style="width:50%;display:table-cell;"><span style="vertical-align:top;font-weight:bold;">User note : </span><span><textarea id="unote" style="font-size:16px;width:60%;height:100px;">'.$_unote.'</textarea></span></div>';
		echo '</div>';
...
?>
		...
		<div align="center" style="width:600px;left:0;right:0;margin-left:auto;margin-right:auto;">
			<?php
					if ($code==0) {
						echo '<button class="order-button" onclick="save()">บันทึก</button>';
							...
							if ($code==1&&$_adminFlg==1) {
								echo '<form action="utility/backward.php" method="post">';
								echo '<button class="order-button" onclick="backward()">ย้อนสถานะ</button>';
			...
			<?php
					if ($code==0) {
							echo '<button class="order-update" onclick="confirm()">ตรวจสอบเรียบร้อย</button>';
					}
			?>
		</div>
		<br>
